...
def get_changed_atoms(reactants, products):
    ...
    #check if there is an underlying isomerization reaction where the only thing that changes
    #is the stereochemistry associated with the molecule. This check only happens after the previous check
    #for chirality results in no results. A number of enzymatic transformations are such isomerization reactions
    if not changed_atom_tags:
        ...
        for (atom_tag, ar, ap) in tetra_atoms:
            ...
            unchanged = (ar.GetChiralTag() == ap.GetChiralTag())
...
def enz_check_leaving_group_stereochem (mol, atoms_to_use, symbols):
    
    #work on a copy of the mol so that the work flow is not affected
    mol_c = deepcopy (mol)
    atoms_to_use_c = deepcopy (atoms_to_use)
    symbols_c = deepcopy (symbols)
    
    #assign fake atom mapping to mol_copy unmapped reactants
    fake_atom_maps = []
    atom_map_0 = 9999
    for atom in mol_c.GetAtoms():
        if not atom.HasProp('molAtomMapNumber'):
            atom_map_0 += 1
            atom.SetAtomMapNum(atom_map_0)
            fake_atom_maps. append ((atom.GetIdx(),atom_map_0))
    
    #set the same atom map numbers in symbols
    for (i, map_number) in fake_atom_maps:
        repl_str = ':'+ str(map_number)+']'
        symbols_c [i] = re.sub (']', repl_str, symbols_c[i])
    ...
    num_tetra_flips = 0
    while not tetra_consistent and num_tetra_flips < 500:
        mol_copy = deepcopy(mol)
        [x.ClearProp('molAtomMapNumber') for x in mol_copy.GetAtoms()]   
        this_fragment = AllChem.MolFragmentToSmiles(mol_copy, atoms_to_use_c, 
            atomSymbols=symbols_c, allHsExplicit=True, 
        ...
        for tetra_map_num in tetra_map_nums:
            ...
            if map_to_id[tetra_map_num] not in all_matched_ids:
                ...
                prevsymbol = symbols_c[map_to_id[tetra_map_num]]
                if '@@' in prevsymbol:
                ...
                symbols_c[map_to_id[tetra_map_num]] = symbol
                num_tetra_flips += 1
    ...
    #remove fake atom maps from symbols_c
    for (i, map_number) in fake_atom_maps:
        org_str = ':'+ str(map_number)+']'
        symbols_c [i] = re.sub (org_str, ']', symbols_c[i])
    
    this_fragment_enz = AllChem.MolFragmentToSmiles(mol_copy, atoms_to_use_c, atomSymbols=symbols_c, 
                                                    allHsExplicit=True, isomericSmiles=USE_STEREOCHEMISTRY, 
                                                    allBondsExplicit=True)
    
    
    
    return this_fragment_enz
...
def extract_from_reaction(reaction):
    ...
    # get length of reactants and products
    len_reactants = len (reaction['reactants'].split('.'))
    len_products = len (reaction['products'].split('.'))
    ...
    if len_products == 1:
        retro_canonical = '(' + retro_canonical.replace ('>>', ')>>')
...
def get_fragments_for_changed_atoms(mols, changed_atom_tags, radius=0, 
    ...
    for mol in mols:
        ...
        tetra_consistent = False
        num_tetra_flips = 0
        while not tetra_consistent and num_tetra_flips < 10:
            mol_copy = deepcopy(mol)
            [x.ClearProp('molAtomMapNumber') for x in mol_copy.GetAtoms()]   
            ...
            for atom in mol.GetAtoms():
                atom.SetIsotope(0)
        if INCLUDE_ALL_UNMAPPED_REACTANT_ATOMS and len(atoms_to_use) > 0:
            this_fragment = enz_check_leaving_group_stereochem (mol, atoms_to_use, symbols)
            tetra_consistent = True # if this was false, the previous step would have raised a value error
